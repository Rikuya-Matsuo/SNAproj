...
bool Animation::Load(const std::string& fileName, bool loop)
{
	...
	for (rapidjson::SizeType i = 0; i < tracks.Size(); i++)
	{
		...
		// tracks[i]の中のboneをuintで読み込み。ボーン番号を取得
		size_t boneIndex = tracks[i]["bone"].GetUint();

		// tracks[i]の中のtransformsが取得できるか？
		...
		for (int j = 0; j < transSize; j++)
		{
			...
			// temp.mRotationにquaternionとしてコピー
...
void Animation::GetGlobalPoseAtTime(std::vector<Matrix4>& outPoses, const Skeleton* inSkeleton, float inTime) const
{
	...
	// これはinTimeが[0～AnimDuration]の間にいることを前提とする。
